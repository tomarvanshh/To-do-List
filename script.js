document.addEventListener("DOMContentLoaded", () => {           // to ensure the DOM is fully loaded before running the script
  ...
  tasks.forEach((element) => { // Loop through each task in the array
    renderTasks(element); // Call the renderTasks function to display each task
  });
  addTaskBtn.addEventListener("click", () => { // Add an event listener to the button for click events
    ...
    console.log(task); // Log the current task array to the console
  });
  function renderTasks(task) { // Function to render a task in the list
  ...
  function saveTask() { // Function to save the task array to localStorage
